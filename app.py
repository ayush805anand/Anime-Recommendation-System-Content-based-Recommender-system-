...
SIM_URL = "https://drive.google.com/uc?id=18o_3kzkC3I3ic-Q9Vsyu25y2Dfs66XsV"
SIM_FILE = "similarity.npy"

if not os.path.exists(SIM_FILE):
    with st.spinner("Downloading similarity.npy (large file)... Please wait."):
        gdown.download(SIM_URL, SIM_FILE, quiet=False)

anime = pickle.load(open("anime_list.pkl", "rb"))
similarity = np.load("similarity.npy", mmap_mode='r')   # <-- memory efficient
...
def recommend(anime_name, category):
    ...
    # distances using memmap row-by-row (low RAM)
